...
const transferAndPayoutToInfluencer = async (id: string) => {

    if (!mongoose.Types.ObjectId.isValid(id)) throw new ApiError(StatusCodes.BAD_REQUEST, 'Invalid Reservation ID');

    const isExistReservation: ICampaign | any = await Campaign.findById(id);
    if (!isExistReservation) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Campaign doesn't exist!");
    }
